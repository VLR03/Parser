...
class Lexer:
    ...
    def make_string(self):
        ...
        # Loop segue até encontrar um '"' ou chegar ao fim do input
        while self.current_char != None and (self.current_char != '"' or escape_character):
            if escape_character:
                # Pega a substituição que deverá ocorrer ao encontrar um '\'
...
class NumberNode:  # pega o Token numérico correspondente (int ou float)
...
class IfNode:
    def __init__(self, cases, else_case):
        ...
        # Pega a primeira posição da lista (tupla) e o primeiro elemento da tupla (expr)
        self.pos_start = self.cases[0][0].pos_start
        # Pega a ultima posição do 'else_case' se ele existir mas se não pega o ultimo elemento da lisa 'cases'
...
class ParseResult:
    ...
    def failure(self, error):
        if not self.error or self.last_registered_advance_count == 0:  # Checa se a função já avançou alguma vez
            self.error = error
        return self
...
class Parser:  # Acompanha pelo index o Token atual
    ...
    def update_current_tok(self):
        if self.tok_idx >= 0 and self.tok_idx < len(self.tokens):  # Se o index do Token estiver em tokens
            self.current_tok = self.tokens[self.tok_idx]  # Pega o token atual
    ...
    def list_expr(self):
        res = ParseResult()
        element_nodes = []
        pos_start = self.current_tok.pos_start.copy()  # Pega a posição inicial do Token atual e faz uma cópia

        if self.current_tok.type != TT_LSQUARE:  # Checa e se o Token atual é um '[' e se não for, retorna erro
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected '['"
            ))
        ...
        # Verifica se a lista é vazia checando se o '[' é seguido diretamente por ']'
    ...
    def while_expr(self):
        ...
        if self.current_tok.type == TT_NEWLINE:
            ...
            body = res.register(self.statements())  # Pega o corpo da expressão e o instancia
            if res.error: return res
    ...
    def bin_op(self, func_a, ops, func_b=None):
        ...
        return res.success(left)  # Retorna o node correto
...
class RTResult:  # Acompanha o resultado atual e qualquer erro também
    def __init__(self):
        self.reset()
    ...
    def register(self, res):  # Verifica se existe um erro e, se sim, atribui a variável
...
class Number(Value):  # Armazena os números e fazer operações com eles
    ...
    def get_comparison_eq(self, other):
        if isinstance(other, Number):  # Checa se o valor é um número
            return Number(int(self.value == other.value)).set_context(self.context), None  # Realiza a operação
        else:
            return None, Value.illegal_operation(self, other)
    ...
    def anded_by(self, other):
        if isinstance(other, Number):  # Checa se o valor é um número
            return Number(int(self.value and other.value)).set_context(self.context), None  # Realiza a operação
        else:
            return None, Value.illegal_operation(self, other)

    def ored_by(self, other):
        if isinstance(other, Number):  # Checa se o valor é um número
            return Number(int(self.value or other.value)).set_context(self.context), None  # Realiza a operação
        else:
            return None, Value.illegal_operation(self, other)

    def notted(self):  # Se o valor é false(0) retorna true(1) e vice-versa
...
class List(Value):
    ...
    def multed_by(self, other):
        if isinstance(other, List):
            new_list = self.copy()
            new_list.elements.extend(other.elements)  # Concatena duas listas
            return new_list, None
        else:
            return None, Value.illegal_operation(self, other)
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_run(self, exec_ctx):
        fn = exec_ctx.symbol_table.get("fn")  # Pega o arquivo e o instancia
        ...
        try:
            with open(fn, "r") as f:  # Leitura do conteúdo do arquivo
                script = f.read()
        except Exception as e:
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                f"Failed to load script \"{fn}\"\n" + str(e),  # Converte a mensagem de erro para string
                exec_ctx
            ))
...
class SymbolTable:
    def __init__(self, parent=None):  # Registra todos os nomes de variáveis e seus valores
        self.symbols = {}
        self.parent = parent

    def get(self, name):  # Pega o valor de uma variável
        value = self.symbols.get(name, None)
        if value == None and self.parent:  # Se o valor for None
...
class Interpreter:
    def visit(self, node, context):  # Interpreta qual método chamar dependendo do tipo do node
        method_name = f'visit_{type(node).__name__}'  # Cria uma string indicando qual método está sendo visitado
        method = getattr(self, method_name, self.no_visit_method)  # Pega o método indicado
    ...
    def visit_IfNode(self, node, context):
        ...
        for condition, expr, should_return_null in node.cases:  # percorre todos os pares de condições e expressões em 'node.cases'
            condition_value = res.register(self.visit(condition, context))  # Instancia os valores das expressões
            if res.should_return(): return res
        ...
        if node.else_case:  # Verifica se o node tem um 'else'
    ...
    def visit_WhileNode(self, node, context):
        ...
        while True:  # Loop infinito
            ...
            if not condition.is_true():  # Se a condição não for verdadeira sai do loop
                break
...
def run(fn, text):
    ...
    # Gerar árvore de sintaxe abstrata
