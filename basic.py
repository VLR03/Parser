...
class Parser:  # Acompanha pelo index o Token atual
    ...
    def advance(self, ):
        self.tok_idx += 1
        if self.tok_idx < len(self.tokens):  # Se o index do Token estiver em tokens
    ...
    def if_expr(self):
        ...
        condition = res.register(self.expr())  # Procura pela expressão que vem antes do 'IF'
        if res.error: return res
        ...
        expr = res.register(self.expr())  # Procura pela expressão que vem depois do 'THEN'
        if res.error: return res
        cases.append((condition, expr))  # Junta na tupla a condição e o que será feito se a condição for satisfeita
...
class Interpreter:
    ...
    def visit_IfNode(self, node, context):
        ...
        for condition, expr in node.cases:  # percorre todos os pares de condições e expressões em 'node.cases'
            condition_value = res.register(self.visit(condition, context))  # Instancia os valores das condições
            if res.error: return res

            if condition_value.is_true():  # Verifica se o valor dessa condição é verdadeiro
                expr_value = res.register(self.visit(expr, context))  # Instancia os valores das expressões
                if res.error: return res
                return res.success(expr_value)

        if node.else_case:  # Verifica se o node tem um 'else'
            else_value = res.register(self.visit(node.else_case, context))  # Instancia os valores da expressão
            if res.error: return res
...
global_symbol_table.set("NULL", Number(0))  # Digitar null é a mesma coisa que digitar 0
